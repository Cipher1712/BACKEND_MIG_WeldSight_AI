...
def _physics_penalty(reconstructed_raw: torch.Tensor) -> torch.Tensor:
    ...
    return torch.stack((
        torch.relu((55.0 - reconstructed_raw[:, 14]) / 55.0),  # stability
        torch.relu((reconstructed_raw[:, 4] - 8.0) / 8.0),     # std
        torch.relu((reconstructed_raw[:, 21] - 0.35) / 0.35),  # short ratio
        torch.relu((reconstructed_raw[:, 23] - 0.90) / 0.10),  # entropy
    ), dim=1)
...
def train_healthy_models(
    ...
    torch.save({
        ...
        "latent_scale": latent_scale, "feature_names": WindowFeatures.names(),
    }, output / "vae.pt")

    reference = {}
    for index, name in enumerate(WindowFeatures.names()):
        reference[name] = {
